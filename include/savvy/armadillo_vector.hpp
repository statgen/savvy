...
{
  template <typename T>
  class armadillo_dense_vector : public arma::Mat<T>
  {
  public:
...
//  template <typename T>
//  using sparse_armadillo_allele_vector = allele_vector<arma::sp_mat<T>>;

  template <typename T>
  using dense_armadillo_allele_vector = allele_vector<armadillo_dense_vector<T>>;
}
