...
namespace vc
{
  reader::reader(const std::string& file_path)
  {
    if (vc::detail::has_extension(file_path, ".cmf"))
      cmf_reader_ = std::make_unique<sav::reader>(file_path);
  ...
  {
    if (vc::detail::has_extension(file_path, ".cmf"))
      cmf_reader_ = std::make_unique<sav::indexed_reader>(file_path, reg);
    else if (detail::has_extension(file_path, ".vcf") || detail::has_extension(file_path, ".vcf.gz") || detail::has_extension(file_path, ".bcf"))
  ...
  {
    if (cmf_reader_)
      dynamic_cast<sav::indexed_reader*>(cmf_reader_.get())->reset_region(reg);
    else if (vcf_reader_)
