  ...
  for (auto it = paths_beg; it != paths_end; ++it)
  {
    ...
    else
    {
      ...
      for (auto it = read_times.begin(); it != read_times.end(); ++it)
      {
        ...
        savvy::sav2::reader rdr(input_path);
        std::size_t cnt = 0;
    ...
    float avg = static_cast<float>(std::accumulate(read_times.begin(), read_times.end(), 0)) / read_times.size();

    std::cout << "\t" << avg << "\t" << input_path << std::endl;
  }
...
savvy::sav2::site_info get_site_info(std::size_t allele_index, bcf_hdr_t* hdr, bcf1_t* rec)
{
